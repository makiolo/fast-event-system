...
{
	{
		...
		Order expected:
		 ...
		 Person A: "5. I'm taking a break from my homework. There seems to be no end to the amount
				...
				asyncply::parallel(vjobs,
					...
					[&]()
					{
						c.connect([&](int iter, const std::string& data)
							{
								std::cout << "recibido en t3 " << data << ", iter " << iter
										  << std::endl;
								++count;
							});
					},
					[&]()
					{
						c.connect([&](int iter, const std::string& data)
							{
								std::cout << "recibido en t4 " << data << ", iter " << iter
										  << std::endl;
								++count;
							});
					});
